...
function PantallaInfoEquipo() {
  ...
  const showMessageButton =
    teamData.membersList.length > 1 && adminId !== userId;
  ...
  const fetchTeams = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/api/equipos/getAll"
      );
    ...
    const fetchTeamData = async () => {
      try {
        ...
        const isMember =
          ...
          data.miembros.some(
            (member) => member && member.usuario && member.usuario.id === userId
          );
        ...
        const availableUsers = allUsers
          .filter(
            (user) => !membersList.some((member) => member.id === user.id)
          )
          .map((user) => ({
            id: user.id,
  ...
  const handleSaveTeamChanges = async (updatedData) => {
    try {
      ...
      const categoria = sportsCategories.find(
        (c) => c.nombre.toLowerCase() === updatedData.sport.toLowerCase()
      );
      ...
      setTeamData((prev) => ({
        ...
        sport:
          updatedTeam.categoriaDeporte?.nombre.toLowerCase() ||
          updatedData.sport,
  ...
  const EditTeamModal = ({ team, onSave, onClose, sportsCategories }) => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (!selectedCategory) {
          throw new Error(
            "Por favor selecciona una categoría de deporte válida"
          );
        }
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          >
            <motion.div style={{ marginBottom: "0.10rem" }}>
              <svg
                width="24"
                height="24"
                viewBox="0 0 14 14"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {teamData.sport.charAt(0).toUpperCase() +
                      teamData.sport.slice(1)}
                  </span>
                </div>
              </div>
          ...
          <div>
                    ...
                    {showTeamOptions && (
                      ...
                      >
                        ...
                        {teamData.isAdmin && (
                          <motion.button
                            whileHover={{
                              backgroundColor: "rgba(255, 35, 39, 0.19)",
                            }}
                            onClick={(e) => {
                              e.stopPropagation();
            ...
            >
              ...
              {teamData.membersList?.map((member, index) => (
                ...
                >
                  {!isMobile && (
                    <div style={{ color: lightTextColor }}>{index + 1}</div>
                  )}
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    ...
                    <div>
                      ...
                      {isMobile && (
                        <div
                          style={{ fontSize: "0.8rem", color: lightTextColor }}
                        >
                          {member.email}
                        </div>
                      )}
                  ...
                  {!isMobile && (
                    <div style={{ color: lightTextColor }}>{member.email}</div>
                  )}
          ...
          >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {Array.isArray(teams) ? (
                          ...
                          >
                            ...
                            <div>
                              ...
                              >
                                Crea nuevos equipos para poder organizar eventos
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        {Array.isArray(teams) &&
                          teams.filter((team) => team.id !== teamData.id)
                            .length === 0 &&
                          teams.length > 0 && (
                            <div
                              style={{
            ...
            >
              ...
              <motion.button
                ...
                disabled={
                  !opponentTeam || !eventDate || !eventTime || !eventLocation
                }
                style={{
                  background:
                    !opponentTeam || !eventDate || !eventTime || !eventLocation
          ...
          >
            ...
            <p style={{ color: lightTextColor, marginBottom: "1.5rem" }}>
              {confirmationAction === "leave"
                ...
                : confirmationAction === "delete"
                ? "Esta acción eliminará el grupo permanentemente. ¿Estás seguro?"
