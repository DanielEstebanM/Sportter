...
function PantallaMensajes() {
    ...
    const cargarConversaciones = async () => {
        ...
        try {
            ...
            const conversacionesRes = await mensajeService.obtenerConversacionesUsuario(userId);
            ...
            // Check for selected conversation after setting the conversations
            if (selectedConversationId) {
                const convToSelect = conversacionesTransformadas.find(
                    c => c.id === selectedConversationId
                );
                if (convToSelect) {
                    setSelectedUser(convToSelect);
                }
            }
            ...
            for (const conv of conversacionesTransformadas) {
                try {
                    const mensajesResponse = await mensajeService.obtenerMensajesConversacion(conv.id);
                    if (mensajesResponse.data?.length > 0) {
                        const ultimoMensaje = mensajesResponse.data[mensajesResponse.data.length - 1];
